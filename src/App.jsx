...
export default function App() {
    ...
    // ✅ SECURITY FIX: Venue portal gating now relies on a Firestore field (isVenueOwner)
    // rather than the URL hostname alone, which is trivially bypassable by the client.
        ...
        currentScreen = (
            <div className="min-h-screen flex items-center justify-center bg-gray-100 dark:bg-gray-900 p-4 text-center">
                <div className="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-xl max-w-md w-full border-t-8 border-red-600">
                    ...
                    <p className="text-gray-600 dark:text-gray-400 mb-6">Your account has been suspended. Please contact support if you believe this is a mistake.</p>
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white animate-fadeIn relative">
            ...
            <main className="max-w-7xl mx-auto px-6 py-16 md:py-24 text-center relative z-10">
                ...
                <button onClick={onLoginClick} className="bg-gradient-to-r from-blue-600 to-purple-600 text-white px-8 py-4 rounded-full font-black text-xl hover:scale-105 transition transform shadow-xl hover:shadow-blue-500/25">Create Your Free Group</button>
            </main>
                ...
                {filteredGroups.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filteredGroups.map(group => (
                            <div key={group.id} onClick={() => handlePreview(group)} className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-xl transition transform hover:-translate-y-1 cursor-pointer group/card">
                            ...
                            <div className="space-y-3">
                                {previewPubs.length === 0 ? <p className="text-gray-500 italic text-center">No pubs rated yet.</p> : previewPubs.map((pub, index) => (
                                    <div key={pub.id} className="flex items-center gap-4 bg-gray-50 dark:bg-gray-700/50 p-3 rounded-xl border border-gray-100 dark:border-gray-600">
                                        ...
                                        <div className="flex-1 min-w-0"><h4 className="font-bold truncate dark:text-white">{pub.name}</h4><p className="text-xs text-gray-500 truncate">{pub.location}</p></div>
                                    </div>
                                ))}
                            </div>
                        )}
...
// ✅ SECURITY FIX: Password validation + friendly error messages
...
function AuthScreen({ auth, onBack }) {
    ...
    const getFriendlyError = (code) => FIREBASE_AUTH_ERRORS[code] || 'Something went wrong. Please try again.';

    const handleAuthAction = async () => {
        setError("");
        // ✅ SECURITY FIX: Frontend password validation
        if (isSignUp) {
            ...
            <div className="bg-white dark:bg-gray-800 p-8 rounded-3xl shadow-2xl max-w-sm w-full relative">
                ...
                <div className="space-y-3">
                    ...
                    <div>
                        ...
                        {isSignUp && <p className="text-xs text-gray-400 mt-1 ml-1">Must be at least 8 characters</p>}
                    </div>
                ...
                {error && <p className="text-red-500 text-sm mt-4 text-center font-bold bg-red-50 dark:bg-red-900/20 p-3 rounded-xl">{error}</p>}
                <p className="text-center text-sm text-gray-500 dark:text-gray-400 mt-6">
                    {isSignUp ? "Already have an account?" : "Don't have an account?"}
                    <button onClick={() => { setIsSignUp(!isSignUp); setError(''); }} className="text-blue-600 dark:text-blue-400 hover:underline ml-1 font-bold">
...
function GroupPortal({ user, userProfile, auth, db }) {
    ...
    useEffect(() => {
        if (!userProfile.groups || userProfile.groups.length === 0) { setLoadingGroups(false); setMyGroups([]); return; }
        setLoadingGroups(true);
        const unsub = db.collection("groups")
            .where(firebase.firestore.FieldPath.documentId(), "in", userProfile.groups)
            .onSnapshot(
                (snapshot) => { setMyGroups(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }))); setLoadingGroups(false); },
                (err) => { console.error("Error fetching group names", err); setError("Could not load your groups."); setLoadingGroups(false); }
            );
        return () => unsub();
    ...
    const handleCreateGroup = async (e) => {
        ...
        setIsCreating(true); setError(""); setMessage("");
        try {
                ...
                if (defaultsSnap.exists && defaultsSnap.data().criteria) {
                    ...
                    defaultsSnap.data().criteria.forEach((crit, index) => {
                        batch.set(newGroupRef.collection('criteria').doc(), {
                            name: crit.name, type: crit.type, weight: crit.weight || 1,
                            archived: false, order: index, createdAt: firebase.firestore.FieldValue.serverTimestamp()
                        });
                    });
                    await batch.commit();
                }
        ...
        } catch (err) { console.error("Error creating group:", err); setError("Failed to create group."); setIsCreating(false); }
    };

    // ✅ SECURITY FIX: handleJoinGroup now queries by a dedicated 'inviteCode' field,
    // NOT the Firestore document ID. Admin must set group.inviteCode in the console.
    // This prevents users from joining groups by guessing document IDs.
    const handleJoinGroup = async (e, directGroupId = null) => {
        if (e) e.preventDefault();
        setError(""); setMessage("");

        // Direct join from public group listing (uses document ID — intentionally public)
        if (directGroupId) {
            try {
                ...
                if (groupData.members?.includes(userProfile.uid)) {
                    await userRef.update({ activeGroupId: directGroupId }); return;
                }
                if (groupData.pendingMembers?.includes(userProfile.uid)) { setMessage("⏳ Your join request is still pending approval."); return; }
                if (groupData.requireApproval) {
                    ...
                    await userRef.update({ groups: firebase.firestore.FieldValue.arrayUnion(directGroupId), activeGroupId: directGroupId });
                    setMessage("🎉 Successfully joined the group!");
                }
            } catch (err) { console.error("Error joining group:", err); setError("Failed to join group."); }
            return;
        }

        // Private invite code: query by inviteCode field, not document ID
        ...
        try {
            ...
            if (groupData.members?.includes(userProfile.uid)) {
                ...
                await userRef.update({ activeGroupId: groupId }); return;
            }
            if (groupData.pendingMembers?.includes(userProfile.uid)) { setMessage("⏳ Your join request is still pending approval."); return; }
            if (groupData.requireApproval) {
                await groupRef.update({ pendingMembers: firebase.firestore.FieldValue.arrayUnion(userProfile.uid) });
                ...
                await groupRef.update({ members: firebase.firestore.FieldValue.arrayUnion(userProfile.uid) });
                await userRef.update({ groups: firebase.firestore.FieldValue.arrayUnion(groupId), activeGroupId: groupId });
                setMessage("🎉 Successfully joined the group!");
            }
            setJoinCode("");
        } catch (err) { console.error("Error joining group:", err); setError("Failed to join group. Please try again."); }
    };
        ...
        <div className="min-h-screen py-10 px-4 transition-colors animate-fadeIn">
            <div className="max-w-6xl mx-auto space-y-8">
                <div className="bg-white dark:bg-gray-800 p-8 rounded-3xl shadow-xl border border-gray-100 dark:border-gray-700 flex flex-col md:flex-row justify-between items-center gap-4">
                    ...
                    <button onClick={handleLogout} className="bg-red-50 text-red-600 dark:bg-red-900/20 dark:text-red-400 border border-red-200 dark:border-red-800 px-6 py-3 rounded-xl font-bold hover:bg-red-100 dark:hover:bg-red-900/40 transition shadow-sm w-full md:w-auto">Log Out</button>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-1 space-y-8">
                        <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-md border border-gray-100 dark:border-gray-700">
                            ...
                            {loadingGroups ? <LoadingScreen text="Loading groups..." /> : myGroups.length === 0 ? (
                                <p className="text-gray-500 dark:text-gray-400 italic text-center py-6">You haven't joined any groups yet.</p>
                            ) : (
                                <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2">
                                    {myGroups.map(group => (
                                        <button key={group.id} onClick={() => handleSelectGroup(group.id)} className="w-full text-left p-4 bg-gray-50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-600 rounded-xl shadow-sm hover:shadow-md transition group flex justify-between items-center">
                        ...
                        <div className="space-y-6 bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-md border border-gray-100 dark:border-gray-700">
                            <div className="space-y-3">
                                ...
                                <form onSubmit={handleJoinGroup} className="flex flex-col gap-2">
                                    <input type="text" value={joinCode} onChange={e => setJoinCode(e.target.value)} placeholder="e.g. PINT-2024" className="px-4 py-3 border dark:border-gray-600 bg-gray-50 dark:bg-gray-700 dark:text-white rounded-xl focus:ring-2 focus:ring-blue-500 outline-none shadow-inner" />
                            ...
                            <div className="space-y-3">
                                ...
                                <form onSubmit={handleCreateGroup} className="flex flex-col gap-2">
                                    <input type="text" value={createGroupName} onChange={e => setCreateGroupName(e.target.value)} placeholder="e.g. The Friday Pint Club" className="px-4 py-3 border dark:border-gray-600 bg-gray-50 dark:bg-gray-700 dark:text-white rounded-xl focus:ring-2 focus:ring-blue-500 outline-none shadow-inner" />
                                    <button type="submit" disabled={isCreating} className="bg-gray-900 dark:bg-white text-white dark:text-gray-900 px-6 py-3 rounded-xl font-bold hover:opacity-80 transition disabled:opacity-50 shadow-sm w-full">{isCreating ? "Creating..." : "Create Group"}</button>
                                </form>
                            </div>
                        </div>
                    </div>
                    <div className="lg:col-span-2 bg-white dark:bg-gray-800 p-6 md:p-8 rounded-3xl shadow-md border border-gray-100 dark:border-gray-700 flex flex-col h-full">
                        <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-8 gap-4 border-b border-gray-100 dark:border-gray-700 pb-6">
                            ...
                            <input type="text" placeholder="Search City..." value={searchCity} onChange={e => setSearchCity(e.target.value)} className="px-4 py-3 border border-gray-200 dark:border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-700 text-sm focus:ring-2 focus:ring-blue-500 outline-none w-full sm:w-64 shadow-inner dark:text-white" />
                        </div>
                        {filteredPublicGroups.length === 0 ? (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 overflow-y-auto pr-2 pb-4">
                                {filteredPublicGroups.map(group => {
                                    ...
                                    return (
                                        <div key={group.id} className="bg-gray-50 dark:bg-gray-700/50 p-5 rounded-2xl border border-gray-200 dark:border-gray-600 transition-colors flex flex-col h-full">
                                            ...
                                            <p className="text-blue-600 dark:text-blue-400 text-xs font-bold tracking-wider uppercase mt-1 mb-4">📍 {group.city || "Global"}</p>
                                            ...
                                            <div className="mt-auto">
                                                {isMember ? (
                                                    <button onClick={() => handleSelectGroup(group.id)} className="w-full py-2.5 bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded-xl font-bold hover:opacity-80 transition shadow-sm">Enter Dashboard</button>
                                                ) : isPending ? (
                                                    <button disabled className="w-full py-2.5 bg-gray-200 dark:bg-gray-600 text-gray-500 dark:text-gray-400 rounded-xl font-bold cursor-not-allowed">⏳ Request Pending</button>
                                                ) : (
                                                    <button onClick={() => handleJoinGroup(null, group.id)} className="w-full py-2.5 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 transition shadow-md">{group.requireApproval ? "Request to Join" : "Join Instantly"}</button>
                                                )}
                                            </div>
                                        </div>
                                    );
