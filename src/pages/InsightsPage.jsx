    ...
    return (
        <div style={{ maxWidth: '56rem', margin: '0 auto' }} className="space-y-8 animate-fadeIn pb-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2" style={{ gap: 'var(--space-6)' }}>
                            ...
                            {analytics.harshCritics.map((critic, i) => (
                                <div key={critic.uid} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)' }}>
                                        <span style={{ fontSize: 'var(--text-lg)' }}>{['🥇','🥈','🥉'][i]}</span>
                                        <div>
                ...
                {/* Most Divisive */}
                <div style={cardStyle}>
                    <h3 style={{ fontSize: 'var(--text-lg)', fontWeight: 900, color: 'var(--color-text)', marginBottom: 'var(--space-1)' }}>⚔️ Most Divisive Pubs</h3>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
                            {analytics.divisivePubs.map((pub, i) => (
                                <div key={pub.name} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                    ...
                                    <span style={{ fontSize: 'var(--text-xs)', fontWeight: 700, background: 'var(--color-brand-highlight)', color: 'var(--color-brand)', padding: 'var(--space-1) var(--space-2)', borderRadius: 'var(--radius-full)' }}>σ² {pub.variance.toFixed(2)}</span>
                                </div>
                            ))}
                        </div>
                    )}
