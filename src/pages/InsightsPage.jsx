...
function KpiCard({ icon, label, value, suffix = '', color, delay = 0 }) {
    ...
    return (
        <div style={{ flex: '1 1 160px', background: 'var(--color-surface)', border: `1.5px solid ${color}33`, borderRadius: 'var(--radius-xl)', padding: 'var(--space-5) var(--space-6)', boxShadow: `0 4px 20px ${color}18, var(--shadow-sm)`, display: 'flex', flexDirection: 'column', gap: 'var(--space-2)', opacity: visible ? 1 : 0, transform: visible ? 'translateY(0)' : 'translateY(16px)', transition: 'opacity 0.5s ease, transform 0.5s ease' }}>
            ...
            <div style={{ fontSize: 'clamp(1.8rem, 3vw, 2.6rem)', fontWeight: 900, fontVariantNumeric: 'tabular-nums', color, fontFamily: 'var(--font-display)', lineHeight: 1 }}>{display}{suffix}</div>
    ...
    return (
        <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-xl)', padding: 'var(--space-6)', boxShadow: 'var(--shadow-md)' }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 'var(--space-4)', marginBottom: 'var(--space-5)' }}>
                <div><h3 style={{ fontSize: 'var(--text-lg)', fontWeight: 900, color: 'var(--color-text)', fontFamily: 'var(--font-display)' }}>Top Ranked Pubs</h3><p style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)', marginTop: 'var(--space-1)' }}>Top 10 by average score — switch category with the tabs.</p></div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--space-2)' }}>{tabs.map(tab => (<button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ padding: 'var(--space-1) var(--space-3)', borderRadius: 'var(--radius-full)', border: activeTab === tab.id ? '2px solid var(--color-brand)' : '1.5px solid var(--color-border)', background: activeTab === tab.id ? 'var(--color-brand)' : 'transparent', color: activeTab === tab.id ? '#fff' : 'var(--color-text-muted)', fontFamily: 'var(--font-body)', fontWeight: 700, fontSize: 'var(--text-xs)', cursor: 'pointer', transition: 'all 180ms ease' }}>{tab.name}</button>))}</div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)', position: 'relative' }}>
                {chartData.map((row, i) => { const pct = (row.score / 10) * 100; const color = tierColor(row.score); return (
                    <div key={row.name} style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)', cursor: 'default' }} onMouseEnter={() => setTooltip({ name: row.name, score: row.score, count: row.count, idx: i })} onMouseLeave={() => setTooltip(null)}>
                        ...
                        <div style={{ flex: 1, height: '22px', background: 'var(--color-surface-2)', borderRadius: 'var(--radius-full)', overflow: 'hidden', position: 'relative' }}>
                            ...
                            {pct > 30 && <span style={{ position: 'absolute', right: '0.5rem', top: '50%', transform: 'translateY(-50%)', fontSize: '11px', fontWeight: 900, color: '#fff', fontVariantNumeric: 'tabular-nums', opacity: animated ? 1 : 0, transition: `opacity 0.3s ease ${0.4 + i * 0.05}s`, pointerEvents: 'none' }}>{row.score.toFixed(1)}</span>}
                        </div>
                ...
                {tooltip !== null && (<div style={{ position: 'absolute', top: `${tooltip.idx * 34 - 6}px`, right: 0, background: 'var(--color-text)', color: 'var(--color-bg)', borderRadius: 'var(--radius-lg)', padding: 'var(--space-2) var(--space-3)', fontSize: 'var(--text-xs)', fontWeight: 700, pointerEvents: 'none', zIndex: 10, whiteSpace: 'nowrap', boxShadow: 'var(--shadow-md)' }}>{tooltip.name} &mdash; <span style={{ fontVariantNumeric: 'tabular-nums' }}>{tooltip.score.toFixed(2)}/10</span> &bull; {tooltip.count} ratings</div>)}
            </div>
            <div style={{ display: 'flex', gap: 'var(--space-4)', marginTop: 'var(--space-5)', flexWrap: 'wrap' }}>
                {[{ color: '#b45309', label: '8.5+ Legendary' }, { color: '#d97706', label: '7.0+ Great' }, { color: '#f59e0b', label: '5.0+ Decent' }, { color: '#fbbf24', label: 'Below 5' }].map(({ color, label }) => (<div key={label} style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-1)' }}><div style={{ width: '10px', height: '10px', borderRadius: '2px', background: color }} /><span style={{ fontSize: '11px', color: 'var(--color-text-muted)', fontWeight: 600 }}>{label}</span></div>))}
            </div>
        </div>
    );
}
    ...
    return (
        <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-xl)', padding: 'var(--space-6)', boxShadow: 'var(--shadow-md)' }}>
            <div style={{ marginBottom: 'var(--space-5)' }}>
                ...
                <p style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)', marginTop: 'var(--space-1)' }}>How your group’s {total.toLocaleString()} individual scores are spread across 1–10. Group mean: <strong style={{ color: 'var(--color-brand)' }}>{mean.toFixed(2)}</strong></p>
            </div>
            <div style={{ position: 'relative', paddingBottom: 'var(--space-6)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-end', gap: '3px', height: '120px' }}>
                    {buckets.map((count, i) => {
                        ...
                        return (
                            <div key={score} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '3px', cursor: 'default', height: '100%', justifyContent: 'flex-end' }} onMouseEnter={() => setHoveredBucket(i)} onMouseLeave={() => setHoveredBucket(null)}>
                                {isHovered && count > 0 && (<div style={{ position: 'absolute', bottom: 'calc(var(--space-6) + 128px)', background: 'var(--color-text)', color: 'var(--color-bg)', borderRadius: 'var(--radius-md)', padding: '4px 8px', fontSize: '11px', fontWeight: 700, whiteSpace: 'nowrap', pointerEvents: 'none', zIndex: 10, boxShadow: 'var(--shadow-md)' }}>Score {score}: {count} ratings ({pct}%)</div>)}
            ...
            <div style={{ display: 'flex', gap: 'var(--space-6)', paddingTop: 'var(--space-4)', borderTop: '1px solid var(--color-border)', flexWrap: 'wrap' }}>
                {[{ label: 'Most common score', value: `${buckets.indexOf(Math.max(...buckets)) + 1}/10` }, { label: 'Scores of 8+', value: `${((buckets.slice(7).reduce((a, b) => a + b, 0) / total) * 100).toFixed(0)}%` }, { label: 'Scores below 5', value: `${((buckets.slice(0, 4).reduce((a, b) => a + b, 0) / total) * 100).toFixed(0)}%` }].map(({ label, value }) => (
                    <div key={label}><p style={{ fontSize: '11px', fontWeight: 600, color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>{label}</p><p style={{ fontSize: 'var(--text-lg)', fontWeight: 900, color: 'var(--color-brand)', fontVariantNumeric: 'tabular-nums' }}>{value}</p></div>
                ))}
            </div>
        </div>
    );
...
/* ── divisive split bar (step 4) ──────────────────────────────────── */
function DivisivePubRow({ pub, index, animated }) {
    // Split bar: left = % of scores <= mean, right = % above mean
    // Use variance to size the spread arms: higher variance = wider split
    const spreadPct = Math.min(50, (pub.variance / 10) * 50); // 0-50% each side
    ...
    return (
        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)' }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {/* split bar — centred, red left / green right */}
                <div style={{ position: 'relative', height: '10px', background: 'var(--color-surface-2)', borderRadius: 'var(--radius-full)', overflow: 'hidden' }}>
                    {/* left (harsh) arm */}
                    <div style={{
                        position: 'absolute', left: 0, top: 0, bottom: 0,
                        width: animated ? `${leftPct}%` : '50%',
                        background: 'linear-gradient(90deg, #dc262688, #dc2626cc)',
                        borderRadius: 'var(--radius-full) 0 0 var(--radius-full)',
                        transition: `width ${0.5 + index * 0.1}s cubic-bezier(0.16, 1, 0.3, 1)`,
                    }} />
                    {/* right (generous) arm */}
                    <div style={{
                        position: 'absolute', right: 0, top: 0, bottom: 0,
                        width: animated ? `${100 - rightPct}%` : '50%',
                        background: 'linear-gradient(90deg, #16a34acc, #16a34a88)',
...
export default function InsightsPage({ pubs, scores, users, criteria }) {
    ...
    return (
        <div style={{ maxWidth: '56rem', margin: '0 auto' }} className="space-y-8 animate-fadeIn pb-20">
            ...
            {/* STEP 4: Rater profiles + divisive split bars */}
            <div className="grid grid-cols-1 md:grid-cols-2" style={{ gap: 'var(--space-6)' }}>

                {/* Harshest Critics → Rater Cards */}
                <div style={cardStyle}>
