...
// Warm amber palette that matches the app's brand
const WHEEL_COLORS = [
...
function drawWheel(canvas, pubs, rotationDeg) {
    ...
    const fontSize = Math.max(9, Math.min(16, Math.floor(radius * 0.115)));
    ...
    pubs.forEach((pub, i) => {
        ...
        const maxChars = Math.max(6, Math.floor(15 - n * 0.35));
        let label = pub.name;
...
export default function SpinTheWheelPage({ pubs, newPubs, criteria, scores }) {
    ...
    // CSS rotation (unbounded for smooth transitions)
    ...
    // Redraw canvas whenever pubs list or settled rotation changes
    useEffect(() => {
    ...
    return (
        <div style={{ maxWidth: '680px', margin: '0 auto', padding: 'var(--space-6) var(--space-4) var(--space-16)' }}>
            <style>{`
                @keyframes wheelGlow {
                ...
                .shimmer-btn {
                    background: linear-gradient(90deg,
                        var(--color-brand) 0%, #fbbf24 40%, var(--color-brand) 80%);
            ...
            <div className="grain-overlay" style={{
                ...
                <h2 style={{ fontFamily:'var(--font-display)', fontSize:'clamp(2rem, 5vw, 3rem)', fontWeight:400, color:'#ffffff', lineHeight:1.1, marginBottom:'var(--space-3)', textShadow:'0 2px 12px rgba(0,0,0,0.25)' }}>
                    Spin the Wheel
                </h2>
            ...
            {/* ── Filters ───────────────────────────────────────────────── */}
            <div style={{ background:'var(--color-surface)', border:'1px solid var(--color-border)', borderRadius:'var(--radius-2xl)', padding:'var(--space-5) var(--space-6)', marginBottom:'var(--space-5)', boxShadow:'var(--shadow-sm)' }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(min(200px,100%),1fr))', gap:'var(--space-4)' }}>
                    ...
                    <div>
                        ...
                        <select value={criterionId} onChange={e => setCriterionId(e.target.value)} style={{
                ...
                >
                    {pubCount > 0 ? (
                        ...
                        <div style={{
                            ...
                            <p style={{ fontFamily:'var(--font-body)', fontWeight:700, fontSize:'var(--text-sm)', color:'var(--color-text-muted)', textAlign:'center', padding:'0 var(--space-8)' }}>
            ...
            {spinResult && !isSpinning && showResult && (() => {
                ...
                return (
                    <div className="result-card grain-overlay" style={{
                        marginTop: 'var(--space-5)',
                        borderRadius: 'var(--radius-2xl)',
                        overflow: 'hidden',
                        border: `2px solid ${tierColor}`,
                        boxShadow: `0 8px 32px ${tierColor}40, var(--shadow-lg)`,
                    }}>
                        {/* Photo or colour banner */}
                        {spinResult.photoURL ? (
                            <img
                                src={spinResult.photoURL}
                                alt={spinResult.name}
                                loading="lazy"
                                width="680" height="220"
                                style={{ width:'100%', height:'14rem', objectFit:'cover', display:'block' }}
                            />
                        ) : (
                            <div style={{
                                height: '10rem',
                                background: `linear-gradient(135deg, ${tierColor}cc 0%, ${tierColor}66 100%)`,
                        ...
                        <div style={{ padding:'var(--space-6) var(--space-7) var(--space-7)', background:'var(--color-surface)' }}>
                            {/* Label */}
                            <p style={{ fontFamily:'var(--font-body)', fontWeight:700, fontSize:'var(--text-xs)', letterSpacing:'0.12em', textTransform:'uppercase', color:'var(--color-brand)', marginBottom:'var(--space-2)' }}>
                                🎉 Tonight's Pub Is…
                            </p>

                            {/* Pub name */}
                            <h3 style={{ fontFamily:'var(--font-display)', fontSize:'clamp(1.8rem,4vw,2.6rem)', fontWeight:400, color:'var(--color-text)', lineHeight:1.1, marginBottom:'var(--space-3)' }}>
                                {spinResult.name}
                            </h3>
                            ...
                            <div style={{ display:'flex', flexWrap:'wrap', gap:'var(--space-3)', alignItems:'center', marginBottom:'var(--space-4)' }}>
                                {/* Location */}
                                {spinResult.location && (
                                    <span style={{ fontFamily:'var(--font-body)', fontSize:'var(--text-sm)', color:'var(--color-text-muted)', display:'flex', alignItems:'center', gap:'var(--space-1)' }}>
                                        📍 {spinResult.location}
                                    </span>
                                )}
                                ...
                                {/* Score badge */}
                                {hasScore && (
                                    <span style={{ background:tierColor, color:'#fff', borderRadius:'var(--radius-full)', padding:'var(--space-1) var(--space-4)', fontFamily:'var(--font-body)', fontWeight:800, fontSize:'var(--text-sm)' }}>
                                        {scoreVal.toFixed(1)}/10
                                    </span>
                                )}

                                {/* Tier badge */}
                                <span style={{ background:`${tierColor}22`, color:tierColor, border:`1px solid ${tierColor}55`, borderRadius:'var(--radius-full)', padding:'var(--space-1) var(--space-3)', fontFamily:'var(--font-body)', fontWeight:700, fontSize:'var(--text-xs)' }}>
                                    {tierLabel}
                                </span>

                                {/* List type */}
                                {spinResult._listType === 'toVisit' && (
                                    <span style={{ background:'var(--color-warning-bg)', color:'var(--color-warning)', border:'1px solid var(--color-warning)', borderRadius:'var(--radius-full)', padding:'var(--space-1) var(--space-3)', fontFamily:'var(--font-body)', fontWeight:700, fontSize:'var(--text-xs)' }}>
                                        📋 On your hit list
                                    </span>
                                )}
                            </div>
                            ...
                            {(spinResult.tags || []).length > 0 && (
                                <div style={{ display:'flex', flexWrap:'wrap', gap:'var(--space-2)', marginBottom:'var(--space-5)' }}>
                                    {spinResult.tags.slice(0, 5).map(tag => (
                                        <span key={tag} style={{ background:'var(--color-surface-2)', border:'1px solid var(--color-border)', borderRadius:'var(--radius-full)', padding:'var(--space-1) var(--space-3)', fontFamily:'var(--font-body)', fontWeight:600, fontSize:'var(--text-xs)', color:'var(--color-text-muted)' }}>
                                            {tag}
                                        </span>
                                    ))}
                                </div>
                            ...
                            <div style={{ display:'flex', gap:'var(--space-3)', flexWrap:'wrap' }}>
                                <button onClick={handleSpin} className="btn-brand">
                                    🔄 Spin Again
                                </button>
                                {spinResult.googleLink && (
                                    <a href={spinResult.googleLink} target="_blank" rel="noopener noreferrer" className="btn-ghost">
                                        📍 Google Maps
                                    </a>
                                )}
                            </div>
