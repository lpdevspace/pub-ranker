    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                {member?.avatarUrl
                    ? <img src={member.avatarUrl} alt={name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} loading="lazy" width="32" height="32" />
                    : initials(name)
                }
            </div>
...
export default function AchievementsPage({
    ...
    // Load crawl count for selected user
    useEffect(() => {
    ...
    // Compute badges for selected user
    ...
    const filteredBadges = useMemo(() => {
        const earned = badges.filter(b => b.earned  && (activeCategory === 'all' || b.category === activeCategory));
        const locked = badges.filter(b => !b.earned && (activeCategory === 'all' || b.category === activeCategory));
        return [...earned, ...locked];
    }, [badges, activeCategory]);

    // Recently unlocked (top 3 earned — no timestamps so use definition order)
    const recentlyUnlocked = earnedBadges.slice(0, 3);

    // Rarest badge (highest tier earned)
    const tierOrder = { legendary: 4, gold: 3, silver: 2, bronze: 1 };
    const rarestBadge = earnedBadges.sort((a, b) => (tierOrder[b.tier] || 0) - (tierOrder[a.tier] || 0))[0];

    // Badge leaderboard — count earned badges per user
        ...
        <div style={{ maxWidth: 900, margin: '0 auto', paddingBottom: '3rem' }}>
            ...
            <div style={{
                ...
                <div style={{ background: 'var(--color-surface)', borderRadius: '1rem', padding: '1rem', textAlign: 'center', border: '1px solid var(--color-border, #e5e7eb)' }}>
                    <p style={{ fontSize: '1.8rem', fontWeight: 900, color: '#d97706', margin: 0, fontVariantNumeric: 'tabular-nums' }}>{earnedBadges.length}</p>
                    ...
                    >
                        ...
                        {cat.id !== 'all' && (
                            <span style={{
                                ...
                                {badges.filter(b => b.earned && b.category === cat.id).length}
                            </span>
                        )}
                    </button>
                ))}
